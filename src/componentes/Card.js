  ...
  return (
    <Fragment>
      <div className="cont-card col-sm-12 col-md-4 m-3 ">
        <h4 className="title-2">{ (main && category) ? `${getMain(category)}: ${main}` : 'No info available'}</h4>
        <p className="title-3 mt-4">{ (info1 && category) ?  `${getInfo1(category)}: ${info1}` : 'No info available'}</p>
        <p className="title-3">{ (info2 && category) ?  `${getInfo2(category)}: ${info2}` : 'No info available'}</p>
        <Btn1 type="button">
          <Link to={urlSw ? `/${getCategoryFromUrl(urlSw)}/${getIdFromUrl(urlSw)}` : ''}>More Details</Link>
        </Btn1>
      </div>
    </Fragment>
  );
